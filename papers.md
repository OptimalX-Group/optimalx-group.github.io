---
title: Papers
Permalink: /papers/
description: "Publications from the OptimalX research group at University of Minnesota, covering optimal control, trajectory optimization, game theory, convex optimization, and autonomous systems led by Prof. Yue Yu."
---

<!-- For those interested in numbers, see Konrad's [google scholar citations profile](https://scholar.google.com/citations?user=MiFqJGcAAAAJ).

We try to include links for all of our papers. Some of the links open PDFs, others direct you to a journal's site where that particular publication is available for download. If you cannot access one of our papers, let us know. The copyright notice for these papers is listed at the bottom of the page. -->
<br>
<h3>Papers</h3>

For metrics and citations, please refer to Yue’s [Google Scholar profile](https://scholar.google.com/citations?user=bwhxFnEAAAAJ&hl=en).


<!-- {% include bibliography.html %} -->

### 2026

***Warm-Started Sequential Convex Programming for Multi-Agent Quadrotor Trajectory Optimization***  
M Yuan, Y Yu  
Journal of Guidance, Control, and Dynamics, 1-9

***Nonlinear Trajectory Optimization Models for Energy-Sharing UAV-UGV Systems with Multiple Task Locations***  
M Yuan, A Adane, J Humann, Y Yu  
arXiv preprint arXiv:2603.28447

### 2025

***MultiNash-PF: A particle filtering approach for computing multiple local generalized Nash equilibria in trajectory games***  
M Bhatt, I Askari, Y Yu, U Topcu, H Fang, N Mehr  
2025 IEEE/RSJ International Conference on Intelligent Robots and Systems …

***Actively Learning to Coordinate in Convex Games via Approximate Correlated Equilibrium***  
Z Fang, A Deshwal, Y Yu  
arXiv preprint arXiv:2509.10989

***Sequential Convex Programming with Filtering-Based Warm-Starting for Continuous-Time Multiagent Quadrotor Trajectory Optimization***  
M Yuan, Y Yu  
arXiv preprint arXiv:2508.14299

***Onboard Dual Quaternion Guidance for Rocket Landing***  
AG Kamath, JA Doll, P Elango, T Kim, S Mceowen, Y Yu, TP Reynolds, ...  
arXiv preprint arXiv:2508.10439

***Filtering-Linearization: A First-Order Method for Nonconvex Trajectory Optimization with Filter-Based Warm-Starting***  
M Yuan, RJ Caverly, Y Yu  
2025 American Control Conference (ACC), 547-552

***Active inverse learning in stackelberg trajectory games***  
W Ward, Y Yu, J Levy, N Mehr, D Fridovich-Keil, U Topcu  
2025 American Control Conference (ACC), 1547-1553

***Newton-PIPG: A Fast Hybrid Algorithm for Quadratic Programs in Optimal Control***  
D Luo, Y Yu, M Fazel, B Açıkmeşe  
arXiv preprint arXiv:2503.22131

### 2024

***Constraint preconditioning and parameter selection for a first-order primal-dual method applied to model predictive control***  
GM Chari, Y Yu, B Açımeşe  
2024 IEEE 63rd Conference on Decision and Control (CDC), 1676-1683

***A coupled optimization framework for correlated equilibria in normal-form games***  
SHQ Li, Y Yu, F Dörfler, J Lygeros  
2024 IEEE 63rd Conference on Decision and Control (CDC), 1739-1744

***Sensing Resource Allocation Against Data-Poisoning Attacks in Traffic Routing***  
Y Yu, AJ Thorpe, J Milzman, D Fridovich-Keil, U Topcu  
2024 IEEE 63rd Conference on Decision and Control (CDC), 8396-8402

***Learning Hyperplanes for Multi-Robot Collision Avoidance in Space***  
F Palafox, Y Yu, D Fridovich-Keil  
AAS/AIAA Astrodynamics Specialist Conference

***Noise-aware and equitable urban air traffic management: An optimization approach***  
Z Gao, Y Yu, Q Wei, U Topcu, JP Clarke  
Transportation Research Part C: Emerging Technologies 165, 104740

***Alpha-fair routing in urban air mobility with risk-aware constraints***  
Y Yu, Z Gao, SHQ Li, Q Wei, JP Clarke, U Topcu  
2024 American Control Conference (ACC), 2248-2253

***Coordination in noncooperative multiplayer matrix games via reduced rank correlated equilibria***  
J Im, Y Yu, D Fridovich-Keil, U Topcu  
IEEE Control Systems Letters 8, 1637-1642

### 2023

***Soft-bellman equilibrium in affine markov games: Forward solutions and inverse learning***  
S Chen, Y Yu, D Fridovich-Keil, U Topcu  
2023 62nd IEEE Conference on Decision and Control (CDC), 2202-2207

***Learning hyperplanes for multi-agent collision avoidance in space***  
F Palafox, Y Yu, D Fridovich-Keil  
arXiv preprint arXiv:2311.09439

***Real-time quadrotor trajectory optimization with time-triggered corridor constraints***  
Y Yu, K Nagpal, S Mceowen, B Açıkmeşe, U Topcu  
Journal of Guidance, Control, and Dynamics 46 (6), 1197-1205

***Poisoning Attacks Against Data-Driven Predictive Control***  
Y Yu, R Zhao, S Chinchali, U Topcu  
2023 American Control Conference (ACC), 545-550

***Adaptive constraint satisfaction for markov decision process congestion games: Application to transportation networks***  
SHQ Li, Y Yu, NI Miguel, D Calderone, LJ Ratliff, B Açıkmeşe  
Automatica 151, 110879

***Vertiport selection in hybrid air–ground transportation networks via mathematical programs with equilibrium constraints***  
Y Yu, M Wang, M Mesbahi, U Topcu  
IEEE Transactions on Control of Network Systems 10 (4), 2108-2119

***Index of papers published in the IEEE Control Systems Letters and presented at the American Control Conference (ACC 2023).***  
G Fedele, L D’Alfonso, A Rodionova, L Lindemann, M Morari, GJ Pappas, ...  
IEEE Control Systems 7, 613-618

***Customized real-time first-order methods for onboard dual quaternion-based 6-DoF powered-descent guidance***  
AG Kamath, P Elango, T Kim, S Mceowen, Y Yu, JM Carson, M Mesbahi, ...  
AIAA scitech 2023 forum, 2003

***Dynamic routing in stochastic urban air mobility networks: A markov decision process approach***  
Q Wei, Y Yu, U Topcu  
IFAC-PapersOnLine 56 (2), 10760-10767

***Real-time sequential conic optimization for multi-phase rocket landing guidance***  
AG Kamath, P Elango, Y Yu, S Mceowen, GM Chari, JM Carson III, ...  
IFAC-PapersOnLine 56 (2), 3118-3125

### 2022

***Inverse matrix games with unique quantal response equilibrium***  
Y Yu, J Salfity, D Fridovich-Keil, U Topcu  
IEEE Control Systems Letters 7, 643-648

***Cost Design in Atomic Routing Games***  
Y Yu, S Chen, D Fridovich-Keil, U Topcu  
arXiv preprint arXiv:2210.01221

***Proportional–integral projected gradient method for conic optimization***  
Y Yu, P Elango, U Topcu, B Açıkmeşe  
Automatica 142, 110359

***Extrapolated proportional-integral projected gradient method for conic optimization***  
Y Yu, P Elango, B Açıkmeşe, U Topcu  
IEEE Control Systems Letters 7, 73-78

***Variable demand and multi-commodity flow in Markovian network equilibrium***  
Y Yu, D Calderone, SHQ Li, LJ Ratliff, B Açıkmeşe  
Automatica 140, 110224

***Inverse matrix games with unique nash equilibrium***  
Y Yu, J Salfity, D Fridovich-Keil, U Topcu  
arXiv preprint arXiv:2207.08275

***A customized first-order solver for real-time powered-descent guidance***  
P Elango, AG Kamath, Y Yu, B Acikmese, M Mesbahi, JM Carson  
AIAA scitech 2022 forum, 0951

***Relationship Design for Socially Desirable Behavior in Static Games.***  
S Chen, YE Bayiz, D Fridovich-Keil, Y Yu, U Topcu  
CoRR

### 2021

***On controllability and persistency of excitation in data-driven control: Extensions of Willems’ fundamental lemma***  
Y Yu, S Talebi, HJ Van Waarde, U Topcu, M Mesbahi, B Açıkmeșe  
2021 60th IEEE Conference on Decision and Control (CDC), 6485-6490

***Proportional-integral projected gradient method for infeasibility detection in conic optimization***  
Y Yu, U Topcu  
arXiv preprint arXiv:2109.02756

***Efficient Algorithms for Convex Optimization based Control***  
Y Yu  
University of Washington

***Advances in trajectory optimization for space vehicle control***  
D Malyuta, Y Yu, P Elango, B Açıkmeşe  
Annual Reviews in Control 52, 282-315

### 2020

***Proportional-integral projected gradient method for model predictive control***  
Y Yu, P Elango, B Açıkmeşe  
IEEE Control Systems Letters 5 (6), 2174-2179

***RC circuits based distributed conditional gradient method***  
Y Yu, B Açıkmeşe  
arXiv preprint arXiv:2003.06949

***RLC circuits-based distributed mirror descent method***  
Y Yu, B Açıkmeşe  
IEEE Control Systems Letters 4 (3), 548-553

***Mass–spring–damper networks for distributed optimization in non-Euclidean spaces***  
Y Yu, B Açıkmeşe, M Mesbahi  
Automatica 112, 108703

### 2019

***Stochastic Bregman parallel direction method of multipliers for distributed optimization***  
Y Yu, B Açıkmeşe  
2019 IEEE 58th Conference on Decision and Control (CDC), 5550-5555

***Tolling for constraint satisfaction in markov decision process congestion games***  
SHQ Li, Y Yu, D Calderone, L Ratliff, B Açrkmeşe  
2019 American Control Conference (ACC), 1238-1243

### 2018

***Controlled markov processes with safety state constraints***  
M El Chamie, Y Yu, B Açıkmeşe, M Ono  
IEEE Transactions on Automatic Control 64 (3), 1003-1018

***Bregman parallel direction method of multipliers for distributed optimization via mirror averaging***  
Y Yu, B Açıkmeşe, M Mesbahi  
IEEE Control Systems Letters 2 (2), 302-306

### 2016

***Decision-Making Policies for Heterogeneous Autonomous Multi-Agent Systems with Safety Constraints.***  
R Zhang, Y Yu, M El Chamie, B Açikmese, DH Ballard  
IJCAI, 546-553

***Convex synthesis of randomized policies for controlled Markov chains with density safety upper bound constraints***  
M El Chamie, Y Yu, B Açıkmeşe  
2016 American Control Conference (ACC), 6290-6295

